...
function showSocialClass(npc, town) {
  if (
    socialClasses[npc.socialClass]
  ) {
    return `${capitalizeFirstLetter(npc.heshe)} belongs to the ${
      npc.socialClass
    } social class.`;
  }
  return '';
...
function showBeard(npc) {
	if (!npc.beard) return '';
	return `, a ${npc.beard}`;
}
function showWeapon(npc) {
	if (!npc.weapon) return '';
	return `[br]In combat ${npc.heshe} uses ${npc.weapon}.`
}
